...
TO run
$ python3 proofOfWork_multi.py 5100
$ python3 proofOfWork_multi.py 5200

TO test
- Nodes no Sync
    $ curl http://localhost:5100/blockchain
    $ curl http://localhost:5200/blockchain
    $ curl http://localhost:5100/mine
    $ curl http://localhost:5100/blockchain
    $ curl http://localhost:5200/blockchain
- To Sync Nodes
    $ curl -H "Content-type: application/json" -d '{"nodes" : ["http://127.0.0.1:5100"]}' -X POST http://localhost:5200/nodes/add_nodes
    $ curl -H "Content-type: application/json" -d '{"nodes" : ["http://127.0.0.1:5200"]}' -X POST http://localhost:5100/nodes/add_nodes
    $ curl http://localhost:5100/nodes/sync
    $ curl http://localhost:5200/nodes/sync
- Send transaction in first and mine with sync 
    $ curl -X POST -H "Content-Type: application/json" -d '{"sender": "04d0988bfa799f7d7ef9ab3de97ef481", "recipient":"cd0f75d2367ad456607647edde665d6f", "amount": 42}' "http://localhost:5100/transactions/new"
    $ curl http://localhost:5100/mine
    $ curl http://localhost:5200/nodes/sync 
    $ curl http://localhost:5200/blockchain
'''
