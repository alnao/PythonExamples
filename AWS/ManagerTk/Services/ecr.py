...
class ConsoleECR:
    ...
    def load_frame(self):
        ...
        self.list=list(service.list_repositories())
        self.list = sorted( self.list , key=lambda tup: tup["repositoryName"] , reverse=False )
        self.list_images=service.list_images
        self.crea_window()
    ...
    def open_detail(self, event): #(frame,profilo,lista_istanze,istanza):
        ...
        dettaglio=None
        for l in self.list:
            if l['repositoryName']==self.el_selezionata:
                dettaglio=l
        ...
        Label(self.frame2, text="Id: " + self.el_selezionata ).pack()
        ...
        i=0
        for key in dettaglio:
            self.tree2.insert(parent='',index='end',iid=i,text='',
                    values=(key,dettaglio[key]) )
            i=i+1
        self.tree2.bind("<Double-1>", self.copy_value2)
        self.tree2.pack()
        self.frame2b = ttk.Frame(self.frame2,height=300)
        if 'repositoryArn' in dettaglio:
            lista_images=self.list_images( self.el_selezionata)
            l_name= Label(self.frame2b, text="Images:")
            l_name.pack()
            ...
            self.tree3.column("Chiave", width=500)
            self.tree3.column("Valore",anchor=CENTER,width=200)
            self.tree3.heading("#0",text="",anchor=CENTER)
            self.tree3.heading("Chiave",text="Chiave",anchor=CENTER)
            self.tree3.heading("Valore",text="Tag",anchor=CENTER)
            i=0
            for valore in lista_images:
                self.tree3.insert(parent='',index='end',iid=i,text='',
                        #values=( str(valore['Key']),str(valore['Value'])) )
                    values=( str(valore['imageDigest']),valore['imageTag']) )
                i=i+1
