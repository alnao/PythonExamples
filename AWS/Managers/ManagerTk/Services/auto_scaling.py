...
class ConsoleAutoScaling:
    ...
    def load_frame(self):
        ...
        self.list=service.list_asgs()
        self.detail=service.describe_asg
#        self.stage_list=service.stage_list
    ...
    def crea_window(self):
        ...
        for dis in self.list:
            nome=dis['AutoScalingGroupName']
            info=str(dis['MinSize'])+"-"+str(dis['DesiredCapacity'])+"-"+str(dis['MaxSize']) #'MinSize': 1, 'MaxSize': 4, 'DesiredCapacity': 1
    ...
    def open_detail(self, event): #(frame,profilo,lista_istanze,istanza):
        ...
        for elemento in self.list:
            if selezionato_id==elemento['AutoScalingGroupName']:
        ...
        Label(self.frame2, text="Nome: " + elemento['AutoScalingGroupName'] ).pack()
        #Label(self.frame2, text="(doppio click sulla risorsa per copiare l'url)" ).pack()
        #servizi
        detail_ob = self.detail(self.selezionato_id)
        ...
        if 'Instances' in detail_ob:
            if len(detail_ob['Instances'])>0:
                ...
                self.tree3.column("Numero", width=200)
