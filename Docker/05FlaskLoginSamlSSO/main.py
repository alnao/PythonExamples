...
url_callback="application" #costante usata dal sso
...
@app.route('/servlet/saml/auth', methods=['GET', 'POST'])
def acs():
    ...
    if 'sso' in request.args:
        ...
        #print("ramo SSO2")
        return_to = request.host_url + url_callback #ex '%sattrs/' % request.host_url
        #print(return_to)
    ...
    elif 'acs' in request.args or ( 'post_data' in req and request.method == 'POST'):
        #print("acs")
        request_id = None
        if 'AuthNRequestID' in session:
            request_id = session['AuthNRequestID']
        ...
        pass #print("SONO nella else")
    if 'samlUserdata' in session:
        #print("SONO samlUserdata")
        paint_logout = True
...
@app.route('/withoutlogin')
def without_login():
    return "Page without login - <a href='/'>Login</a>"

@app.route('/application')
def application():
    if 'samlUserdata' not in session:
        return make_response(" <a href='/'>Login</a> required", 403 )
    return render_template('application.html', paint_logout=True)
