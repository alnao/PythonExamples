# chapter 2 : Eth examples in python
...
class Blockchain(object):
    ...
    def __init__(self):
        self.nodes = set() #v2 added to sync functionality
    ...
    def add_node(self, address): #v2 added to sync functionality
        parsed_url = urlparse(address)
        self.nodes.add(parsed_url.netloc)
        print(parsed_url.netloc)
       
    # determine if a given blockchain is valid
    def valid_chain(self, chain): #v2 added to sync functionality
    ...
    def update_blockchain(self): #v2 added to sync functionality
...
def add_nodes():  #v2 added to sync functionality
...
def sync():  #v2 added to sync functionality
...
if __name__ == '__main__':
    app.run(host='0.0.0.0', port= int(sys.argv[1]) ) #int(sys.argv[1])

"""
To run and test with only 1 node
$ python3 main.py 5002
$ curl http://localhost:5002/blockchain
$ curl http://localhost:5002/mine
$ curl http://localhost:5002/blockchain
$ curl -X POST -H "Content-Type: application/json" -d '{ "sender": "04d0988bfa799f7d7ef9ab3de97ef481", "recipient":"cd0f75d2367ad456607647edde665d6f", "amount": 5}' "http://localhost:5002/transactions/new"
$ curl http://localhost:5002/mine
$ curl http://localhost:5002/blockchain

To run and test with 2 nodes
$ python3 main.py 5002
$ python3 main.py 5003
$ curl -H "Content-type: application/json" -d '{"nodes" :["http://127.0.0.1:5002"]}' -X POST http://localhost:5003/nodes/add_nodes
$ curl -H "Content-type: application/json" -d '{"nodes" :["http://127.0.0.1:5003"]}' -X POST http://localhost:5002/nodes/add_nodes
$ curl -X POST -H "Content-Type: application/json" -d '{ "sender": "04d0988bfa799f7d7ef9ab3de97ef481", "recipient":"cd0f75d2367ad456607647edde665d6f", "amount": 5}' "http://localhost:5002/transactions/new"
$ curl http://localhost:5002/nodes/sync
    > Our blockchain is the latest
$ curl http://localhost:5003/nodes/sync
    > The blockchain has been updated to the latest
$ curl http://localhost:5002/blockchain
$ curl http://localhost:5003/blockchain
"""